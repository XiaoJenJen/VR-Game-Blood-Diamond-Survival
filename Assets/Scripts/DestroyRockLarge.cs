...
public class DestroyRockLarge : MonoBehaviour {

	public GameObject stoneCrash, Diamond1, Diamond2, Diamond3;
	...
	void Start () {
		...
		RockHits = 0;
		DiamondNumber = Random.Range(1, GeneralController.Instance.DiamondRockLarge);
    ...
    {	
        ...
        if (other.tag == "hand"){
			...
			//Larger rocks require 6 hits to break
			if(RockHits == 6){
				...
				for(int i = 0; i < DiamondNumber; i++){
					...
					if(DiamondType == 3){
						Instantiate(Diamond3, gameObject.transform.position, Quaternion.identity);
					}
				}
